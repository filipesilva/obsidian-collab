...
- Unit tests: `npm test` runs the fast tests in headless Chromium, which has no Node globals, like Obsidian mobile. `npm run test-deps` downloads the browser once. `npm run test-network` runs `src/network.test.ts` and `src/nat.test.ts`, which take a few minutes: it boots the signalling Worker on port 8788 and a local STUN and TURN server on port 3479 (`test/turn-server.mjs`, credentials collab/collab), so those run offline too, relay tests included, and it also enables the tests that go through public Nostr relays. The test browser runs with mDNS candidate hiding off so the local relay can reach real addresses.
